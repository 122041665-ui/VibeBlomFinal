<x-app-layout>
    ...
    @php
        ...
        $isAuthError = $errorText !== '' && (
            str_contains($errorTextLower, 'sesión') ||
            str_contains($errorTextLower, 'session') ||
            str_contains($errorTextLower, 'token') ||
            str_contains($errorTextLower, 'login') ||
            str_contains($errorTextLower, 'auth')
        );
    @endphp
    ...
    <div class="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100/70 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 relative overflow-hidden">
        ...
        <div class="{{ $container }}">
                ...
                <div class="rounded-[28px] border border-gray-100 dark:border-slate-800 bg-white/85 dark:bg-slate-900/85 backdrop-blur shadow-sm p-5 sm:p-6 lg:p-7">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <h1 class="{{ $title }} mt-4">Mis favoritos</h1>
                ...
                <div class="{{ $card }} text-center">
                    <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">
                        <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                            ...
                            <rect x="4" y="11" width="16" height="10" rx="2.5" stroke="currentColor" stroke-width="1.8"/>
                            <path d="M12 15.25v2.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
                        </svg>
                    </div>
                    ...
                    <p class="mt-3 max-w-2xl mx-auto text-sm leading-6 text-gray-600 dark:text-slate-400">
                        No hay una sesión activa en la API para sincronizar los lugares guardados.
                        Una vez iniciada la sesión, aquí aparecerán los favoritos asociados a la cuenta.
                    </p>
            ...
            @else
                @if (!empty($error))
                    <div class="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">
                        {{ $error }}
                    </div>
                @endif
                ...
                @if ($favorites->isEmpty())
                    <div class="{{ $card }} text-center">
                        <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">
                            <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                <path d="M12 20s-7-4.4-9.3-8.5C.7 8.2 2.2 5.3 6 4.8c2-.3 3.7.7 4.7 2c1-1.3 2.7-2.3 4.7-2c3.8.5 5.3 3.4 3.3 6.7C19 15.6 12 20 12 20Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </div>
                        ...
                        <p class="mt-2 max-w-xl mx-auto text-sm leading-6 text-gray-600 dark:text-slate-400">
                            Explora lugares y usa el corazón para guardarlos aquí. Esta sección ayuda a regresar rápido a los sitios que más interesan.
                        </p>
                ...
                @else
                    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div class="inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50/80 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-300 w-fit">
                            <span class="inline-block h-2 w-2 rounded-full bg-blue-500"></span>
                            {{ $favorites->count() }} {{ $favorites->count() === 1 ? 'lugar guardado' : 'lugares guardados' }}
                        </div>

                        <div class="text-xs text-gray-500 dark:text-slate-400">
                            Click en una card para abrir el detalle
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                        @foreach ($favorites as $favorite)
                            @php
                                ...
                                $typeKey = mb_strtoupper($typeRaw !== '' ? $typeRaw : 'OTRO', 'UTF-8');
                                $typeKey = strtr($typeKey, ['Á'=>'A','É'=>'E','Í'=>'I','Ó'=>'O','Ú'=>'U','Ü'=>'U','Ñ'=>'N']);
                                ...
                                if (isset($aliases[$typeKey])) $typeKey = $aliases[$typeKey];

                                $typeIcon = $typeIcons[$typeKey] ?? $typeIcons['OTRO'];
                                $typeLabel = $placeType ?: 'Sin tipo';

                                $rating = max(0, min(5, (int) $placeRating));
                            @endphp
                                    ...
                                    <div class="absolute top-3 right-3 z-20">
                                        <form action="{{ route('favorite.toggle', $placeId) }}"
                                            ...
                                            <button type="submit"
                                                ...
                                                <span class="pointer-events-none absolute -inset-1 rounded-full bg-blue-500/15 opacity-0 group-hover/heart:opacity-100 transition"></span>

                                                <svg class="relative w-7 h-7 text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
